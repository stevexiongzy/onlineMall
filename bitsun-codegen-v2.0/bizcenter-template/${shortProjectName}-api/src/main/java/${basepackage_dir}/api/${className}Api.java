...
public interface ${className}Api {
     ...
     * @return 返回新创建的资源对象
     */
    @ApiOperation(value = "创建一个资源对象（返回新创建的资源对象）", tags = {"${classNameLower}"}, nickname = "add${className}")
    @ApiResponses(value = {@ApiResponse(code = 200, message = "000000:成功，否则失败")})
    ResultDTO<${className}ResDto> add${className}(${className}ReqDto reqDto);
...
<#list table.columns as column>
     ...
     * @return 返回单独的资源对象
     */
    @ApiOperation(value = "查询一个资源对象(若不唯一则随机返回符合条件的一个)", tags = {"${classNameLower}"}, nickname = "getOne${className}ByMap")
    @ApiResponses(value = {@ApiResponse(code = 200, message = "操作是否成功,000000:成功，否则失败")})
    @ApiImplicitParams({
<#list table.columns as column>
    <#if  column.columnNameLower != "id"
     ...
     * @return 返回完整的资源对象
     */
    @ApiOperation(value = "更新一个资源对象(更新参数的全部属性)", tags = {"${classNameLower}"}, nickname = "update${className}ByMap")
    @ApiImplicitParams({
        <#list table.columns as column>
        <#if  column.columnNameLower != "id"
        &&column.columnNameLower != "remarks"
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "操作是否成功,000000:成功，否则失败")})
    ResultDTO<${className}ResDto> update${className}(Long id, Map<String, Object> params);
    /**
     * 更新一个资源对象(部分更新)
     *
     * @return 返回完整的资源对象
